...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
	...
	//------------------------------------------------------------------------
	public void CorreccionBugSkins(){
		if (nextButton)
    ...
    void Start() {
		...
		if (GameManager.Instance.SkinSelection) {
			_pageCount = Mathf.CeilToInt (_container.childCount / 2);
		} else {
			_pageCount = _container.childCount;
		}
		//Debug.Log ("hago el start");
	...
	public void PublicSetPage(int page){
		LerpToPage (page);
	}
